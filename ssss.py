...
def get_config(access_token):
    ...
    def get_next_expiry_internal():
        try:
            ...
            params = {"instrument_key": quote(config['instrument_key'])}  # URL encode instrument_key
            res = requests.get(url, headers=config['headers'], params=params, timeout=10)
            if res.status_code == 200:
                data = res.json()
                if data.get("status") != "success":
                    st.error(f":warning: Error fetching expiries: {data.get('message', 'Unknown error')}")
                    return datetime.now().strftime("%Y-%m-%d")
                expiries = sorted(data.get("data", []), key=lambda x: datetime.strptime(x["expiry"], "%Y-%m-%d"))
                today = datetime.now()
...
def fetch_option_chain(config):
    try:
        ...
        params = {
            "instrument_key": quote(config['instrument_key']),  # URL encode
        ...
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.error(f":warning: Error fetching option chain: {data.get('message', 'Unknown error')}")
                return []
...
def get_indices_quotes(config):
    try:
        ...
        params = {"instrument_key": quote("NSE_INDEX|India VIX,NSE_INDEX|Nifty 50")}  # URL encode
        res = requests.get(url, headers=config['headers'], params=params, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.error(f":warning: Error fetching indices quotes: {data.get('message', 'Unknown error')}")
                return None, None
            vix = data["data"].get("NSE_INDEX:India VIX", {}).get("last_price")
            nifty = data["data"].get("NSE_INDEX:Nifty 50", {}).get("last_price")
            if vix is None or nifty is None:
                st.error(":warning: Missing VIX or Nifty data in response")
                return None, None
            return vix, nifty
...
def get_funds_and_margin(config):
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.warning(f":warning: Error fetching funds and margin: {data.get('message', 'Unknown error')}")
...
def calculate_strategy_margin(config, strategy_details):
    try:
        ...
        headers = {**config['headers'], "Content-Type": "application/json"}  # Explicit Content-Type
        res = requests.post(url, headers=headers, json={"instruments": instruments}, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.warning(f":warning: Failed to calculate margin: {data.get('message', 'Unknown error')}")
                return 0
            total_margin = sum(item.get("total_margin", 0) for item in data.get("data", []))
            return total_margin
...
def place_multi_leg_orders(config, orders):
    try:
        ...
        url = f"{config['base_url']}/order/multi"
        headers = {**config['headers'], "Content-Type": "application/json"}  # Explicit Content-Type
        res = requests.post(url, headers=headers, json=payload, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.error(f":x: Failed to place multi-leg order: {data.get('message', 'Unknown error')}")
                return False
...
def create_gtt_order(config, instrument_token, trigger_price, transaction_type="SELL", tag="SL"):
    try:
        url = f"{config['base_url'].replace('v2', 'v3')}/order/gtt"
        headers = {**config['headers'], "Content-Type": "application/json", "Api-Version": "3.0"}  # Update to v3
        payload = {
            "type": "SINGLE",
        ...
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.warning(f":warning: GTT failed: {data.get('message', 'Unknown error')}")
                return False
...
def _iron_condor_calc(option_chain, spot_price, config, lots):
    ...
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"], "quantity": lots * config["lot_size"], "transaction_type": "SELL"},
        {"instrument_key": pe_short_opt["instrument_key"], "quantity": billots * config["lot_size"], "transaction_type": "SELL"},
        {"instrument_key": ce_long_opt["instrument_key"], "quantity": lots * config["lot_size"], "transaction_type": "BUY"},
        {"instrument_key": pe_long_opt["instrument_key"], "quantity": lots * config["lot_size"], "transaction_type": "BUY"}
    ]
...
def get_option_greeks(config, instrument_keys):
    try:
        if not instrument_keys:
            return {}
        url = f"{config['base_url'].replace('v2', 'v3')}/market-quote/option-greeks"
        headers = {**config['headers'], "Api-Version": "3.0"}  # Update to v3
        params = {"instrument_key": quote(",".join(instrument_keys))}  # URL encode
        res = requests.get(url, headers=headers, params=params, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.warning(f":warning: Error fetching Greeks: {data.get('message', 'Unknown error')}")
                return {}
            return data.get("data", {})
...
def evaluate_full_risk(trades_df, config, regime_label, vix):
    try:
        total_funds = config.get('total_funds', 2000000) or 2000000  # Fallback to 2,000,000
...
def fetch_trade_data(config, full_chain_df):
    try:
        ...
        res_positions = requests.get(url_positions, headers=config['headers'], timeout=10)
        url_trades = f"{config['base_url']}/trade/portfolio-trades"
        res_trades = requests.get(url_trades, headers=config['headers'], timeout=10)
        positions = []
        trades = []
        if res_positions.status_code == 200:
            data = res_positions.json()
            if data.get("status") == "success":
                positions = data.get("data", [])
            else:
                st.warning(f":warning: Error fetching positions: {data.get('message', 'Unknown error')}")
        else:
            st.warning(f":warning: Error fetching positions: {res_positions.status_code} - {res_positions.text}")
        if res_trades.status_code == 200:
            data = res_trades.json()
            if data.get("status") == "success":
                trades = data.get("data", [])
            else:
                st.warning(f":warning: Error fetching trades: {data.get('message', 'Unknown error')}")
        else:
...
def exit_all_positions(config):
    try:
        url = f"{config['base_url']}/portfolio/positions/exit"
        headers = {**config['headers'], "Content-Type": "application/json"}  # Explicit Content-Type
        res = requests.post(url, headers=headers, json={"segment": "EQ"}, timeout=10)
        if res.status_code == 200:
            data = res.json()
            if data.get("status") == "success":
                ...
                return order_ids
            st.error(f":x: Unexpected response status: {data.get('message', 'Unknown error')}")
            return []
...
def logout(config):
    try:
        url = f"{config['base_url']}/user/logout"
        res = requests.delete(url, headers=config['headers'], timeout=10)
        if res.status_code == 200:
            data = res.json()
            if data.get("status") != "success":
                st.error(f":x: Logout failed: {data.get('message', 'Unknown error')}")
                return
...
if st.sidebar.button("Login"):
    if access_token:
        ...
        try:
            res = requests.get(test_url, headers=config['headers'], timeout=10)
...
if st.session_state.logged_in and access_token:
    ...
    config['total_funds'] = funds_data.get('total_funds', 2000000)  # Update with actual funds
    ...
    def load_all_data(config):
        ...
        spot_price = option_chain[0]["underlying_spot_price"] if option_chain else None
    ...
    with tab3:
        ...
        if strategies:
            ...
            st.info("No strategies recommended based on current market conditions.")

    with tab4:
        st.subheader("Portfolio Summary")
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(f"<div class='metric-box'><h3>Total Funds</h3><div class='value'>₹{portfolio_summary['Total Funds']:,.2f}</div></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='metric-box'><h3>Exposure</h3><div class='value'>{portfolio_summary['Exposure Percent']:.2f}%</div></div>", unsafe_allow_html=True)
        with col2:
            st.markdown(f"<div class='metric-box'><h3>Risk on Table</h3><div class='value'>₹{portfolio_summary['Risk on Table']:,.2f}</div></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-box'><h3>Risk Percent</h3><div class='value'>{portfolio_summary['Risk Percent']:.2f}%</div></div>", unsafe_allow_html=True)
        ...
        with col3:
            st.markdown(f"<div class='metric-box'><h3>Realized P&L</h3><div class='value'>₹{portfolio_summary['Realized P&L']:,.2f}</div></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-box'><h3>Drawdown</h3><div class='value'>₹{portfolio_summary['Drawdown ₹']:,.2f} ({portfolio_summary['Drawdown Percent']:.2f}%)</div></div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-box'><h3>Max Drawdown Allowed</h3><div class='value'>₹{portfolio_summary['Max Drawdown Allowed']:,.2f}</div></div>", unsafe_allow_html=True)
        
        st.subheader("Strategy Risk Breakdown")
        if not strategy_df.empty:
            st.dataframe(strategy_df.style.set_properties(**{"background-color": "#1A1C24", "color": "white"}).format({
                "Capital Used": "₹{:,.2f}",
                "Cap Limit": "₹{:,.2f}",
                "% Used": "{:.2f}%",
                "Potential Risk": "₹{:,.2f}",
        ...
        if portfolio_summary["Flags"]:
            st.warning("Portfolio Risk Alerts: " + "; ".join(portfolio_summary["Flags"]))
    ...
    with tab5:
        st.subheader("Place Custom Orders")
        st.warning(":warning: Ensure you understand the risks before placing orders.")
        selected_strategy = st.selectbox("Select Strategy", list(config['risk_config'].keys()))
        lots = st.number_input("Number of Lots", min_value=1, value=1, step=1)
        stop_loss_pct = st.number_input("Stop Loss (%)", min_value=0.0, max_value=100.0, value=5.0, step=0.5)
        
        if st.button("Calculate Order Details"):
            detail = get_strategy_details(selected_strategy, option_chain, spot_price, config, lots)
            if detail:
                st.session_state.order_details = detail
                st.session_state.stop_loss_pct = stop_loss_pct
                order_df = pd.DataFrame({
                    "Instrument": [order["instrument_key"] for order in detail["orders"]],
                    "Type": [order["transaction_type"] for order in detail["orders"]],
                    "Quantity": [abs(order["quantity"]) for order in detail["orders"]],
                    "Price": [order.get("current_price", 0) for order in detail["orders"]]
                })
                st.dataframe(order_df.style.set_properties(**{"background-color": "#1A1C24", "color": "white"}).format({"Price": "₹{:.2f}"}), use_container_width=True)
                margin = calculate_strategy_margin(config, detail)
                st.markdown(f"<div class='metric-box'><h4>Estimated Margin Required</h4> ₹{margin:,.2f}</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='metric-box'><h4>Max Profit</h4> ₹{detail.get('max_profit', 0):,.2f}</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='metric-box'><h4>Max Loss</h4> ₹{detail.get('max_loss', float('inf')):,.2f}</div>", unsafe_allow_html=True)
        ...
        if st.button("Place Order") and "order_details" in st.session_state:
            detail = st.session_state.order_details
            success = place_multi_leg_orders(config, detail["orders"])
            if success:
                if st.session_state.stop_loss_pct > 0:
                    for order in detail["orders"]:
                        if order["transaction_type"] == "SELL":
                            ltp = order.get("current_price", 0)
                            trigger_price = ltp * (1 + st.session_state.stop_loss_pct / 100)
                            create_gtt_order(config, order["instrument_key"], trigger_price, "SELL", f"SL_{selected_strategy}")
                st.success(f":white_check_mark: Placed {selected_strategy} order with {lots} lots!")
            else:
        ...
        if st.button("Exit All Positions"):
            ...
            if order_ids:
                st.success(f":white_check_mark: Exit initiated for {len(order_ids)} positions.")
            else:
                st.info("No positions exited or no open positions found.")

    with tab6:
        st.subheader("Risk Management Dashboard")
        st.markdown(f"<div class='metric-box'><h3>Available Margin</h3><div class='value'>₹{funds_data['available_margin']:,.2f}</div></div>", unsafe_allow_html=True)
        ...
        if not trades_df.empty:
            st.dataframe(trades_df[["strategy", "capital_used", "potential_loss", "realized_pnl", "trades_today", "vega"]].style.set_properties(**{"background-color": "#1A1C24", "color": "white"}).format({
                "capital_used": "₹{:,.2f}",
                "potential_loss": "₹{:,.2f}",
                "realized_pnl": "₹{:,.2f}",
                "vega": "{:.2f}"
        ...
        if portfolio_summary["Flags"]:
            ...
            st.success(":white_check_mark: No risk alerts at this time.")
    ...
    st.info("Please enter your Upstox Access Token in the sidebar to log in and access the dashboard.")
