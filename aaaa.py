...
with st.sidebar:
    ...
    if st.session_state.logged_in:
        try:
            user_profile_res = requests.get(f"{config['base_url']}/user/profile", headers=config['headers'])
...
if st.session_state.logged_in:
    ...
    with col_header1:
        st.markdown(f"<h2>📈 Market Insights Dashboard • Expiry: {config['expiry_date']}</h2>", unsafe_allow_html=True)
    with col_header2:
        last_updated = datetime.now().strftime("%H:%M:%S")
        st.markdown(f"<p><strong>Last Updated:</strong> {last_updated}</p>", unsafe_allow_html=True)
    with col_header3:
    ...
    with tab5:
        ...
        selected_strategy = st.selectbox("Select Strategy", all_strategies, key="order_strategy")
    ...
    with tab6:
        ...
        plot_drawdown_trend(portfolio_summary)
        st.markdown(f"<div class='metric-box'><h4>Current Drawdown</h4> ₹{portfolio_summary['Drawdown â‚¹']:.2f} ({portfolio_summary['Drawdown Percent']:.2f}%)</div>", unsafe_allow_html=True)
...
# Show order status if any
if st.session_state.order_status:
    st.info(st.session_state.order_status)
    st.session_state.order_status = ""
