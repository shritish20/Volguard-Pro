...
def fetch_trade_data(config, full_chain_df):
    try:
        url_positions = f"https://{config['base_url'][8:]}/portfolio/short-term/positions"
...
def plot_drawdown_trend(portfolio_summary):
    ...
    fig.add_hline(
        y=-portfolio_summary["Max Drawdown Allowed"],
        line_dash="dash",
        line_color="red",
        name="Max Drawdown"
    )
...
def main():
    ...
    .stButton > button { 
        background-color: #00BFFF; color: white; border-radius: 5px; padding: 10px 20px; 
    }
    ...
    regime_score, regime, regime_note, regime_explanation = calculate_regime(
        seller.get('avg_iv', 0), ivp, hv_7, garch_7d, 
        seller.get('straddle_price', 0), spot_price, market['pcr'], vix, iv_skew_slope
    )
    
    strategies, strategy_rationale, event_warning = suggest_strategy(
        regime, ivp, iv_rv_spread, market['days_to_expiry'], event_df, 
        config['expiry_date'], seller.get('straddle_price', 0), spot_price
    )
