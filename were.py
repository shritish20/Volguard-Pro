...
if st.session_state.logged_in and access_token:
    ...
    # OPTION CHAIN ANALYSIS TAB
        ...
        # Step 1: Let user override expiry inside this tab
        ...
        # Step 2: Plot IV Skew vs OI with ATM marker
        if not chain_df.empty:
            fig, ax1 = plt.subplots(figsize=(12, 6))
            ax1.plot(chain_df["Strike"], chain_df["IV Skew"], color="#00BFFF", label="IV Skew")
            ax1.axvline(seller['strike'], color="gray", linestyle="--", label="ATM Strike")
            ax1.set_xlabel("Strike")
            ax1.set_ylabel("IV Skew", color="#00BFFF")
            ax2 = ax1.twinx()
            ax2.plot(chain_df["Strike"], chain_df["Total OI"], color="#FF6347", label="Total OI")
            ax2.set_ylabel("Total OI", color="#FF6347")
        ...
        st.dataframe(filtered_df.style.set_properties(**{"background-color": "#1A1C24", "color": "white"}), use_container_width=True)

        # Step 4: Add visual greeks metrics
        st.subheader("📈 Visual Greeks Analysis")
        greek_cols = ["Strike", "Call IV", "Put IV", "Total Theta", "Total Vega", "Straddle Price"]
        greeks_df = filtered_df[greek_cols]

        fig, axs = plt.subplots(3, 1, figsize=(12, 12))
        axs[0].bar(greeks_df["Strike"], greeks_df["Call IV"], color="#1f77b4", label="Call IV")
        axs[0].bar(greeks_df["Strike"], greeks_df["Put IV"], color="#ff7f0e", alpha=0.6, label="Put IV")
        axs[0].legend()
        axs[0].set_title("Call vs Put IV")

        axs[1].bar(greeks_df["Strike"], greeks_df["Total Theta"], color="#2ca02c")
        axs[1].set_title("Total Theta")

        axs[2].bar(greeks_df["Strike"], greeks_df["Total Vega"], color="#d62728")
        axs[2].set_title("Total Vega")

        st.pyplot(fig)

        # Step 5: Download Option Chain CSV
