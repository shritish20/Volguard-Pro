...
# --- Configuration ---
def get_config(access_token=None):
    config = {
        "access_token": access_token or "eyJ0eXAiOiJKV1QiLCJrZXlfaWQiOiJza192MS4wIiwiYWxnIjoiSFMyNTYifQ.eyJzdWIiOiIzUUNUTVMiLCJqdCI6IjY4M2I4YmFmNWEwZmIwNmU1N2I4NzBhNCIsImlzTXVsdGlDbGllbnQiOmZhbHNlLCJpc1BsdXNQbGFuIjp0cnVlLCJpYXQiOjE3NDg3MzI4NDcsImlzcyI6InVkYXBpLWdhdGV3YXktc2VydmljZSIsImV4cCI6MTc0ODgxNTIwMH0.s6qb4uAyaDjQ6nkncK8KtBSojRXGZC8x1whJlO5mHOI",  # Replace with your actual Upstox API token
...
def calculate_strategy_margin(config, orders_payload):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(
                f"Could not find 'used_margin' in the response data properties for margin calculation: {data}")
            return None
        print(f"Error calculating margin: {res.status_code} - {res.text}")
        return None
    except requests.exceptions.RequestException as req_e:
        print(
            f"Network or API request error in calculate_strategy_margin: {req_e}")
        return None
    except Exception as e:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data:
                ...
                print(
                    f"Could not find 'used_margin' or 'available_margin' in the response: {data}")
                return None
        ...
        print(
            f"Network or API request error in get_user_funds_and_margin: {req_e}")
        return None
    except Exception as e:
        print(
            f"Exception in get_user_funds_and_margin: {e}")
        return None
...
def get_order_book(config):
    ...
    try:
        ...
        print(
            f"Network or API request error in get_order_book: {req_e}")
        return []
    except Exception as e:
...
def load_upcoming_events(config):
    try:
        ...
        df["Datetime"] = df["Datetime"].apply(lambda dt: dt.replace(year=current_year) if pd.notnull(
            dt) and dt.month >= datetime.now().month else (
            dt.replace(year=current_year + 1) if pd.notnull(dt) else dt))
        now = datetime.now()
        expiry_dt = datetime.strptime(config['expiry_date'], "%Y-%m-%d")
        mask = (df["Datetime"] >= now) & (
                    df["Datetime"] <= expiry_dt + timedelta(days=1))  # Add a day to include expiry day
...
def market_metrics(option_chain, expiry_date):
    try:
        ...
        possible_strikes = range(int(min_strike), int(max_strike) + 1,
                                 50)  # Assuming 50 point strikes for Nifty
        max_pain = 0
        min_loss = float('inf')
...
def calculate_volatility(config, seller):
    try:
        ...
        if not forecast.variance.empty and forecast.variance.iloc[
            -1] is not None:
...
def calculate_iv_skew_slope(full_chain_df):
    try:
        ...
        df_filtered = full_chain_df.replace([np.inf, -np.inf],
                                           np.nan).dropna(
            subset=["IV Skew", "Strike"])
        if len(df_filtered) < 2:
            return 0  # Need at least two points for regression
        slope, _, _, _, _ = linregress(df_filtered["Strike"],
                                       df_filtered["IV Skew"])
        return slope
    except Exception as e:
...
def calculate_regime(atm_iv, ivp, realized_vol, garch_vol, straddle_price,
                     spot_price, pcr, vix, iv_skew_slope):
    try:
...
def suggest_strategy(regime_label, ivp, iv_minus_rv, days_to_expiry, event_df,
                     expiry_date, straddle_price, spot_price):
    strategies = []
    rationale = []
    event_warning = None
    ...
    for _, row in event_df.iterrows():
        ...
        if level == "High" and pd.notnull(row.get("Forecast")) and pd.notnull(
                row.get("Prior")):
            try:
                ...
                forecast = float(forecast_val) if forecast_val.replace('.',
                                                                      '',
                                                                      1).isdigit() else None
                prior = float(prior_val) if prior_val.replace('.',
                                                              '',
                                                              1).isdigit() else None
                if forecast is not None and prior is not None and abs(
                        forecast - prior) > 0.5:
    ...
    if event_impact_score > 0:
        rationale.append(
            f"High-impact events with significant forecast deviations ({event_impact_score} events).")
    ...
    if regime_label == "🔥 High Vol Trend":
        if high_impact_event_near or event_impact_score > 0:
            ...
            rationale.append(
                "High volatility with major event — use defined-risk structures.")
        else:
            strategies = ["Iron Fly", "Wide Strangle"]
            rationale.append(
                "Strong IV premium — neutral strategies for premium capture.")
    elif regime_label == "🚧 Elevated Volatility":
        strategies = ["Iron Condor", "Jade Lizard"]
        rationale.append(
            "Volatility above average — range-bound strategies offer favorable reward-risk.")
    elif regime_label == "😴 Neutral Volatility":
        if days_to_expiry >= 3:
            strategies = ["Jade Lizard", "Bull Put Spread"]  # Slight directional bias
            rationale.append(
                "Market balanced — slight directional bias strategies offer edge.")
        else:  # Shorter expiry in neutral vol
            strategies = ["Iron Fly", "ATM Strangle"]
            rationale.append(
                "Tight expiry — quick theta-based capture via short Iron Fly/ATM Strangle.")
    elif regime_label == "💤 Low Volatility":
        if days_to_expiry > 7:
            strategies = ["Straddle", "Calendar Spread"]  # Benefit from potential IV increase
            rationale.append(
                "Low IV with longer expiry — benefit from potential IV increase or directional move.")
        else:
            strategies = ["Straddle", "ATM Strangle"]  # Still premium collection
            rationale.append(
                "Low IV — premium collection favorable but monitor for breakout risk.")
    ...
    if event_impact_score > 0 and not high_impact_event_near:
        ...
        strategies_for_events = [s for s in strategies if
                                 "Iron" in s or "Lizard" in s or "Spread" in s]
        if strategies_for_events:  # Only replace if there are defined-risk options
            strategies = strategies_for_events
            rationale.append(
                "Prioritizing defined-risk due to potential impact from upcoming events.")
    if ivp > 85 and iv_minus_rv > 5:
        rationale.append(
            f"Volatility overpriced (IVP: {ivp}%, IV-RV: {iv_minus_rv}%) — ideal for selling premium.")
    elif ivp < 30:
        rationale.append(
            f"Volatility underpriced (IVP: {ivp}%) — consider avoiding unhedged selling or look for long vega plays.")
    rationale.append(
        f"Expected move: ±{expected_move_pct:.2f}% based on straddle price.")
...
def fetch_trade_data(config, full_chain_df):
    try:
        ...
        if res_positions.status_code == 200 and res_trades.status_code == 200:
            positions = res_positions.json()["data"] if res_positions.json().get(
                "data") else []
            trades = res_trades.json()["data"] if res_trades.json().get(
                "data") else []
            trade_counts = {}
            for trade in trades:
                # Basic heuristic: identify strategy from instrument type. Refine as needed.
                instrument_key = trade.get("instrument_key", "")
                if "NIFTY" in instrument_key and (
                        "CE" in instrument_key or "PE" in instrument_key):
            ...
            for pos in positions:
                ...
                if "NIFTY" in instrument and (
                        "CE" in instrument or "PE" in instrument):
                ...
                res_brokerage = requests.get(url_brokerage,
                                             headers=config['headers'],
                                             params=params_brokerage)
                brokerage = res_brokerage.json()["data"]["charges"][
                    "total"] if res_brokerage.status_code == 200 and res_brokerage.json().get(
                    "data") else 0
                # Vega for the position from full_chain_df (assuming it matches)
                # This is a rough estimation, ideally you'd track Vega per leg
                position_vega = 0
                matching_strikes = full_chain_df[full_chain_df["Strike"].apply(
                    lambda x: abs(x - pos.get("strike_price",
                                              0)) < 1)]  # Find nearest strike
                if not matching_strikes.empty:
                    position_vega = matching_strikes["Total Vega"].iloc[
                                         0] * abs(quantity) / config[
                                         "lot_size"]  # Vega per lot
            ...
            return pd.DataFrame(
                trades_df_list) if trades_df_list else pd.DataFrame()
        print(
            f"Error fetching positions: {res_positions.status_code} - {res_positions.text}")
        print(
            f"Error fetching trades: {res_trades.status_code} - {res_trades.text}")
        return pd.DataFrame()
    except Exception as e:
...
def evaluate_full_risk(trades_df, config, regime_label, current_available_capital):
    try:
        daily_risk_limit = config[
                                'daily_risk_limit_pct'] * config[
                                'total_capital']  # Use total_capital from config, updated by API if needed
        weekly_risk_limit = config['weekly_risk_limit_pct'] * config[
            'total_capital']
        strategy_summary = []
        total_cap_used = 0
        total_risk_used = 0
        ...
        if trades_df.empty:
            portfolio_summary = {
                ...
                "Risk on Table": 0,
                "Risk %": 0,
                "Available Funds": current_available_capital,
        ...
        for _, row in trades_df.iterrows():
            ...
            cfg = config['risk_config'].get(strat, {
                "capital_pct": 0.05,
                "risk_per_trade_pct": 0.01})  # Default values if strat not in config
            ...
            if not risk_ok:
                flags.append(
                    f"❌ {strat} exceeded risk limit (Risk: {potential_loss:.2f}, Limit: {max_risk:.2f})")
            if sl_hit and trades_today > 3:  # Example condition for potential revenge trading
                flags.append(
                    f"⚠️ {strat} shows possible revenge trading (SL hit + {trades_today} trades)")
        net_dd = -total_realized_pnl if total_realized_pnl < 0 else 0
        exposure_pct = round(total_cap_used / config['total_capital'] * 100,
                             2) if config['total_capital'] != 0 else 0
        risk_pct = round(total_risk_used / config['total_capital'] * 100,
                         2) if config['total_capital'] != 0 else 0
        dd_pct = round(net_dd / config['total_capital'] * 100,
                       2) if config['total_capital'] != 0 else 0
        portfolio_summary = {
            "Total Capital": config['total_capital'],
...
def iron_fly(option_chain, spot_price, config, lots=1):
    ...
    instrument_keys = [ce_short_opt["instrument_key"],
                       pe_short_opt["instrument_key"],
                       ce_long_opt["instrument_key"],
                       pe_long_opt["instrument_key"]]
    greeks_data = get_option_greeks(config, instrument_keys)
    ce_short_price = greeks_data.get(ce_short_opt["instrument_key"], {}).get(
        "last_price", ce_short_opt["market_data"]["ltp"])
    pe_short_price = greeks_data.get(pe_short_opt["instrument_key"], {}).get(
        "last_price", pe_short_opt["market_data"]["ltp"])
    ce_long_price = greeks_data.get(ce_long_opt["instrument_key"], {}).get(
        "last_price", ce_long_opt["market_data"]["ltp"])
    pe_long_price = greeks_data.get(pe_long_opt["instrument_key"], {}).get(
        "last_price", pe_long_opt["market_data"]["ltp"])
    premium = (ce_short_price + pe_short_price - ce_long_price - pe_long_price) * lots
    max_loss = (wing - (premium / (
                config["lot_size"] * lots))) * config["lot_size"] * lots
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "SELL", "product": "NRML"},
        {"instrument_key": pe_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "SELL", "product": "NRML"},
        {"instrument_key": ce_long_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "BUY", "product": "NRML"},
        {"instrument_key": pe_long_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "BUY", "product": "NRML"}
    ]
    pricing = {
    ...
    return {"strategy": "Iron Fly",
            "strikes": [strike, strike, strike + wing, strike - wing],
            "premium": premium, "max_loss": max_loss,
            "max_profit": premium, "orders": orders, "pricing": pricing}
...
def iron_condor(option_chain, spot_price, config, lots=1):
    ...
    if not all([ce_short_opt, pe_short_opt, ce_long_opt, pe_long_opt]):
        ...
        return None
    instrument_keys = [opt["instrument_key"] for opt in
                       [ce_short_opt, pe_short_opt, ce_long_opt,
                        pe_long_opt]]
    greeks_data = get_option_greeks(config, instrument_keys)
    ...
    max_loss = ((long_wing_distance - short_wing_distance) - (
                premium / (config["lot_size"] * lots))) * config[
                   "lot_size"] * lots
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
    ...
    return {"strategy": "Iron Condor",
            "strikes": [ce_short_strike, pe_short_strike, ce_long_strike,
                        pe_long_strike],
            "premium": premium, "max_loss": max_loss,
            "max_profit": premium, "orders": orders, "pricing": pricing}
...
def jade_lizard(option_chain, spot_price, config, lots=1):
    ...
    if not all([ce_short_opt, pe_short_opt, pe_long_opt]):
        ...
        return None
    instrument_keys = [opt["instrument_key"] for opt in
                       [ce_short_opt, pe_short_opt, pe_long_opt]]
    greeks_data = get_option_greeks(config, instrument_keys)
    ce_short_price = greeks_data.get(ce_short_opt["instrument_key"], {}).get(
        "last_price", ce_short_opt["market_data"]["ltp"])
    pe_short_price = greeks_data.get(pe_short_opt["instrument_key"], {}).get(
    ...
    max_loss = ((put_short_strike - put_long_strike) - (
                pe_short_price - pe_long_price)) * config["lot_size"] * lots
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "SELL", "product": "NRML"},
...
def straddle(option_chain, spot_price, config, lots=1):
    ...
    max_loss = float(
        "inf")  # Unlimited loss potential for short straddle
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "SELL", "product": "NRML"},
...
def calendar_spread(option_chain, spot_price, config, lots=1):
    ...
    print(
        "Warning: Calendar Spread requires fetching options from multiple expiries. This implementation is a placeholder.")
    atm = min(option_chain, key=lambda x: abs(x["strike_price"] - spot_price))
    strike = atm["strike_price"]
    ...
    ce_long_opt = find_option_by_strike(option_chain, strike,
                                       "CE")  # This should be far expiry, but uses same chain
    if not all([ce_short_opt, ce_long_opt]):  # This check will likely fail or return same opt
        print(
            "Error: Missing options for Calendar Spread. Ensure multi-expiry data is fetched.")
        return None
    ...
    premium = (ce_short_price - ce_long_price) * config[
        "lot_size"] * lots  # Short premium - Long premium (should be negative if long costs more)
    # Max loss is typically the net debit paid
    max_loss = abs(premium)  # Initial debit paid
    max_profit = float(
        "inf")  # Theoretically unlimited, but capped by time decay difference and IV expansion
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "SELL", "product": "NRML"},
    ...
    return {"strategy": "Calendar Spread",
            "strikes": [strike, strike],
            "premium": premium, "max_loss": max_loss,
            "max_profit": max_profit, "orders": orders, "pricing": pricing}


def bull_put_spread(option_chain, spot_price, config, lots=1):
    atm = min(option_chain,
              key=lambda x: abs(x["strike_price"] - spot_price))
    # Short OTM Put
    ...
    if not all([pe_short_opt, pe_long_opt]):
        ...
        return None
    instrument_keys = [opt["instrument_key"] for opt in
                       [pe_short_opt, pe_long_opt]]
    greeks_data = get_option_greeks(config, instrument_keys)
    pe_short_price = greeks_data.get(
        pe_short_opt["instrument_key"], {}).get("last_price",
                                               pe_short_opt["market_data"][
                                                   "ltp"])
    pe_long_price = greeks_data.get(
        pe_long_opt["instrument_key"], {}).get("last_price",
                                             pe_long_opt["market_data"][
                                                 "ltp"])
    premium = (pe_short_price - pe_long_price) * lots  # Net credit received
    # Max loss = difference in strikes - premium received
    max_loss = ((short_strike - long_strike) - (
                premium / (config["lot_size"] * lots))) * config[
                   "lot_size"] * lots
    orders = [
        {"instrument_key": pe_short_opt["instrument_key"],
    ...
    return {"strategy": "Bull Put Spread",
...
def wide_strangle(option_chain, spot_price, config, lots=1):
    ...
    instrument_keys = [opt["instrument_key"] for opt in
                       [ce_short_opt, pe_short_opt]]
    greeks_data = get_option_greeks(config, instrument_keys)
    ce_short_price = greeks_data.get(
        ce_short_opt["instrument_key"], {}).get("last_price",
                                               ce_short_opt["market_data"][
                                                   "ltp"])
    pe_short_price = greeks_data.get(
        pe_short_opt["instrument_key"], {}).get("last_price",
                                              pe_short_opt["market_data"][
    ...
    premium = (ce_short_price + pe_short_price) * lots
    max_loss = float(
        "inf")  # Unlimited loss potential for short strangle
    orders = [
        {"instrument_key": ce_short_opt["instrument_key"],
         "quantity": lots * config["lot_size"],
         "transaction_type": "SELL", "product": "NRML"},
...
def plot_chain_analysis(full_chain_df):
    ...
    # IV Skew
    sns.lineplot(data=full_chain_df, x="Strike", y="IV Skew",
                 ax=axes[0, 0], marker="o", color="purple")
    axes[0, 0].set_title("IV Skew Across Strikes")
    axes[0, 0].axhline(0, linestyle='--', color="gray")
    # Total Theta
    sns.lineplot(data=full_chain_df, x="Strike", y="Total Theta",
                 ax=axes[0, 1], marker="o", color="green")
    axes[0, 1].set_title("Total Theta Across Strikes")
    # Straddle Price
    sns.lineplot(data=full_chain_df, x="Strike", y="Straddle Price",
                 ax=axes[1, 0], marker="o", color="orange")
    axes[1, 0].set_title("Straddle Price Across Strikes")
    # Total OI
    sns.barplot(x=full_chain_df["Strike"].astype(str),
                y=full_chain_df["Total OI"], ax=axes[1, 1],
                palette="Blues_d")
...
def plot_payoff_diagram(strategy_details, spot_price):
    ...
    for detail in strategy_details:
        ...
        for order in detail["orders"]:
            ...
            qty = order["quantity"]
            transaction_type = order["transaction_type"]
            # Find the strike from detail['strikes'] using index for simplicity
            try:
                ...
                strike = detail["strikes"][order_index]  # Weak assumption
            except (ValueError, IndexError):
                print(
                    f"Warning: Could not determine strike for instrument {instrument_key}. Skipping payoff for this leg.")
                continue
            ...
            if is_call:
                payoff_per_share = (strikes - strike) - price if is_buy else price - (
                            strikes - strike)
                payoff_per_share = np.maximum(0, payoff_per_share) if is_buy else np.minimum(
                    0, payoff_per_share)
            else:  # Put option
                payoff_per_share = (strike - strikes) - price if is_buy else price - (
                            strike - strikes)
                payoff_per_share = np.maximum(0, payoff_per_share) if is_buy else np.minimum(
                    0, payoff_per_share)
            # Total payoff for this leg (per unit, then multiplied by quantity and lot size)
            payoffs += payoff_per_share * (abs(qty) / config["lot_size"])
        plt.plot(strikes, payoffs, label=detail["strategy"])
    plt.axvline(spot_price, linestyle="--", color="gray",
...
def main(config):
    ...
    if funds_info:
        ...
        print(
            f"💰 Account Total Capital (Approx): ₹{config['total_capital']:,.2f}")
    ...
    for strat in strategies:
        strat_clean = strat.replace("(hedged)", "").replace("with strict stop",
                                                           "").replace(
            "short ", "").strip()
        func_map = {
            "Iron Fly": iron_fly,
    ...
    return (option_chain, spot_price, seller, hv_7, garch_7d, iv_rv_spread,
