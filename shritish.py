...
if st.session_state.logged_in and access_token:
    ...
    # OPTION CHAIN ANALYSIS TAB
        ...
        st.markdown("### 🔍 ATM ±300 Option Chain View")
        atm_strike = seller['strike']
        chain_filtered = full_chain_df[(full_chain_df['Strike'] >= atm_strike - 300) & (full_chain_df['Strike'] <= atm_strike + 300)]
        calls = chain_filtered[['Strike', 'Call IV']].rename(columns={'Call IV': 'IV'}).assign(Type='Call')
        puts = chain_filtered[['Strike', 'Put IV']].rename(columns={'Put IV': 'IV'}).assign(Type='Put')
        combined = pd.concat([calls, puts]).pivot(index='Strike', columns='Type', values='IV').reset_index()
        atm_mask = combined['Strike'] == atm_strike
        combined['Color'] = '#FFFFFF'
        combined.loc[atm_mask, 'Color'] = '#FFD700'  # Highlight ATM
        fig, ax = plt.subplots(figsize=(14, 8))
        for i, row in combined.iterrows():
            color = row['Color']
            ax.annotate(row['Strike'], xy=(row['Call IV'], row['Strike']), va='center', ha='right', color=color, weight='bold')
            ax.annotate(row['Strike'], xy=(row['Put IV'], row['Strike']), va='center', ha='left', color=color, weight='bold')
            ax.plot([row['Call IV'], row['Put IV']], [row['Strike'], row['Strike']], color='gray', lw=0.8)
        ax.set_title("📈 Call vs Put Implied Volatility Around ATM", fontsize=16, color='white')
        ax.set_xlabel("Implied Volatility (%)", color='white')
        ...
        st.markdown("### 📈 Charts for Option Seller")
        col1, col2 = st.columns(2)
        with col1:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(full_chain_df['Strike'], full_chain_df['IV Skew'], color="#00BFFF")
            ax.axvline(x=atm_strike, color='yellow', linestyle='--', label='ATM Strike')
            ax.set_title("IV Skew Across Strikes", color='white')
            ax.set_xlabel("Strike", color='white')
            ax.set_ylabel("IV Skew", color='white')
            ax.tick_params(colors='white')
            ax.grid(True, linestyle='--', alpha=0.5)
            ax.legend()
            ax.set_facecolor('#0E1117')
        ...
        with col2:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(full_chain_df['Strike'], full_chain_df['Straddle Price'], color="#FF6347")
            ax.axvline(x=atm_strike, color='yellow', linestyle='--', label='ATM Strike')
            ax.set_title("Straddle Price Across Strikes", color='white')
            ax.set_xlabel("Strike", color='white')
            ax.set_ylabel("Straddle Price", color='white')
            ax.tick_params(colors='white')
            ax.grid(True, linestyle='--', alpha=0.5)
            ax.legend()
            ax.set_facecolor('#0E1117')
        ...
        with col3:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(full_chain_df['Strike'], full_chain_df['Total Theta'], color="#32CD32")
            ax.axvline(x=atm_strike, color='yellow', linestyle='--', label='ATM Strike')
            ax.set_title("Total Theta Across Strikes", color='white')
            ax.set_xlabel("Strike", color='white')
        ...
        with col4:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(full_chain_df['Strike'], full_chain_df['Total OI'], color="#FFD700")
            ax.axvline(x=atm_strike, color='yellow', linestyle='--', label='ATM Strike')
            ax.set_title("Open Interest Across Strikes", color='white')
            ax.set_xlabel("Strike", color='white')
            ax.set_ylabel("Total OI", color='white')
            ax.tick_params(colors='white')
            ax.grid(True, linestyle='--', alpha=0.5)
            ax.legend()
            ax.set_facecolor('#0E1117')
        ...
        with col5:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(full_chain_df['Strike'], full_chain_df['Total Theta'] / full_chain_df['Total Vega'], color="#00BFFF")
            ax.axvline(x=atm_strike, color='yellow', linestyle='--', label='ATM Strike')
            ax.set_title("Theta / Vega Ratio Across Strikes", color='white')
            ax.set_xlabel("Strike", color='white')
            ax.set_ylabel("Theta / Vega", color='white')
            ax.tick_params(colors='white')
            ax.grid(True, linestyle='--', alpha=0.5)
            ax.legend()
            ax.set_facecolor('#0E1117')
        ...
        with col6:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(full_chain_df['Strike'], full_chain_df['Total Vega'], color="#4682B4")
            ax.axvline(x=atm_strike, color='yellow', linestyle='--', label='ATM Strike')
            ax.set_title("Total Vega Across Strikes", color='white')
            ax.set_xlabel("Strike", color='white')
    ...
    # STRATEGY SUGGESTIONS TAB
        ...
        st.markdown(f"""
        <div style='padding: 15px; background-color: #1A1C24; border-radius: 10px;'>
            <h3 style='color: #00BFFF;'>🎯 Recommended Strategies</h3>
            <p><strong>Regime:</strong> {regime}<br/>
               <strong>Score:</strong> {regime_score:.2f}<br/>
               <strong>Note:</strong> {regime_note}<br/>
               <i>{regime_explanation}</i></p>
        </div>
        """, unsafe_allow_html=True)

        if strategies:
            st.markdown(f"<div style='padding: 10px; background-color: #1A1C24; border-radius: 8px; margin-top: 10px;'><strong>Suggested Strategies:</strong> {', '.join(strategies)}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='padding: 10px; background-color: #1A1C24; border-radius: 8px; margin-top: 10px;'><strong>Rationale:</strong> {strategy_rationale}</div>", unsafe_allow_html=True)
            if event_warning:
                st.warning(event_warning)
            for strat in strategies:
                st.markdown(f"<h4 style='color: #00BFFF;'>{strat}</h4>", unsafe_allow_html=True)
