...
if st.session_state.logged_in and access_token:
    ...
    with tab3:
        ...
        if strategy_details:
            ...
            if updated_detail:
                ...
                order_df = pd.DataFrame
                    {"Instrument": order["instrument_key"],
                     ...
                     "LTP": order.get("current_price", 0)} for order in updated_detail["orders"]st.dataframe(
            ...
            with col_r1:
                st.markdown(
                    f"<div class='small-metric-box'><h4>💰 Premium:</h4> ₹{detail['premium_total']:.2f}</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<div class='small-metric-box'><h4>📉 Max Loss:</h4> ₹{detail['max_loss']:.2f}</div>",
                    unsafe_allow_html=True,
                )
            with col_r2:
                st.markdown(
                    f"<div class='small-metric-box'><h4>📈 Max Profit:</h4> ₹{detail['max_profit']:.2f}</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<div class='small-metric-box'><h4>🎯 Risk/Reward:</h4> {detail['max_loss'] / detail['max_profit']:.2f}</div>" if detail['max_profit'] > 0 else "<div class='small-metric-box'><h4>🎯 Risk/Reward:</h4> Undefined</div>",
                    unsafe_allow_html=True,
                )
            with col_r3:
                capital_required = detail['premium_total'] * 10  # Approx margin
                st.markdown(
                    f"<div class='small-metric-box'><h4>💸 Capital Required:</h4> ₹{capital_required:.2f}</div>",
                    unsafe_allow_html=True,
                )
                ...
                if st.button(f"Place {strat} Order", key=f"place_{strat}"):
                    detail = get_strategy_details(strat, option_chain, spot_price, config, lots)
                    if detail:
                        for order in detail["orders"]:
...
with tab4:
        ...
        with col_p1:
            st.markdown(
                f"<div class='metric-box'><h3>💰 Total Capital</h3><div class='value'>₹{portfolio_summary['Total Capital']:.2f}</div></div>",
                unsafe_allow_html=True,
            )
        with col_p2:
            st.markdown(
                f"<div class='metric-box'><h3>📈 Capital Deployed</h3><div class='value'>₹{portfolio_summary['Capital Deployed']:.2f}</div></div>",
                unsafe_allow_html=True,
            )
        with col_p3:
            st.markdown(
                f"<div class='metric-box'><h3>⚖️ Exposure %</h3><div class='value'>{portfolio_summary['Exposure %']:.2f}%</div></div>",
                unsafe_allow_html=True,
            )
        with col_p4:
            st.markdown(
                f"<div class='metric-box'><h3>📉 Drawdown %</h3><div class='value'>{portfolio_summary['Drawdown %']:.2f}%</div></div>",
                unsafe_allow_html=True,
            )
...
with tab5:
        ...
        if selected_strategy:
            ...
            if detail:
                ...
                order_df["Strike"] = detail["strikes"]
                order_df["Type"] = order_df["instrument_key"].apply(lambda x: "CE" if "CE" in x else "PE")
                order_df = order_df[["Strike", "Type", "transaction_type", "quantity", "current_price"]]
                order_df.columns = ["Strike", "Option Type", "Action", "Quantity", "Current Price"]
                st.dataframe(
                    order_df.style.format(precision=2).set_properties(
                        **{"background-color": "#1A1C24", "color": "white"}
                    ),
                ...
                st.markdown(
                    f"<div class='metric-box'><h3>💰 Total Premium:</h3><div class='value'>₹{detail['premium_total']:.2f}</div></div>",
                    unsafe_allow_html=True,
                )
...
with tab6:
        ...
        with col_r1:
            st.markdown(
                f"<div class='metric-box'><h3>📉 Total Risk</h3><div class='value'>₹{portfolio_summary['Risk on Table']:.2f}</div></div>",
                unsafe_allow_html=True,
            )
        with col_r2:
            st.markdown(
        ...
        with col_r4:
            st.markdown(
                f"<div class='metric-box'><h3>💸 Margin Utilization</h3><div class='value'>{funds_data['used_margin'] / funds_data['total_funds'] * 100:.2f}%</div></div>",
                unsafe_allow_html=True,
            )
        st.subheader("Greeks Exposure")
        total_delta, total_theta, total_vega, total_gamma = 0, 0, 0, 0
        for pos in positions:
        ...
        with col_g1:
            st.markdown(
                f"<div class='small-metric-box'><h4>📐 Delta</h4>{total_delta:.2f}</div>",
                unsafe_allow_html=True,
            )
        with col_g2:
            st.markdown(
                f"<div class='small-metric-box'><h4>⏳ Theta</h4>₹{total_theta:.2f}</div>",
                unsafe_allow_html=True,
            )
        with col_g3:
            st.markdown(
        ...
        with col_g4:
            st.markdown(
                f"<div class='small-metric-box'><h4>⚡ Gamma</h4>{total_gamma:.6f}</div>",
                unsafe_allow_html=True,
            )
        st.subheader("Capital Allocation")
        plot_allocation_pie(strategy_df, config)
        ...
        if portfolio_summary["Drawdown %"] > (0.05 if vix > 20 else 0.02):
            st.error(f"⚠️ Drawdown ({portfolio_summary['Drawdown %']:.2f}%) exceeds limit!")
        ...
        if positions:
            ...
            st.dataframe(
                pos_df.style.format(precision=2).set_properties(
                    **{"background-color": "#1A1C24", "color": "white"}
                ),
                use_container_width=True,
            )
