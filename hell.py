...
def get_config(access_token):
    ...
    def get_all_expiries(config):
        try:
            ...
            if res.status_code == 200:
                expiries = sorted(res.json()["data"], key=lambda x: datetime.strptime(x["expiry"], "%Y-%m-%d"))
                return list(dict.fromkeys([e["expiry"] for e in expiries]))
            return []
        except Exception as e:
            st.warning(f":warning: Failed to load expiry list: {e}")
            return []

    config['get_all_expiries'] = get_all_expiries
...
def load_upcoming_events(config):
    try:
        ...
        df["Datetime"] = pd.to_datetime(
            ...
            format="%d-%b %H:%M", errors="coerce"
        )

        # Fix year if default year (1900) is detected
