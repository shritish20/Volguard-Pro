...
# --- Helper Functions ---
def determine_hedge_width_factor(ivp, iv, garch, regime_label):
    factor = 1.0
    if ivp > 85 or regime_label == ":fire: High Vol Trend":
        factor = 2.0
    elif ivp > 65 or regime_label == ":zap: Elevated Volatility":
        factor = 1.5
    elif ivp < 30 or regime_label == ":chart_with_downwards_trend: Low Volatility":
        factor = 0.75
    else:
        factor = 1.0
...
def get_strategy_details(strategy_name, option_chain, spot_price, config, lots=1):
    ...
    if detail:
        # Build a map of instrument_key -> LTP from option_chain
        ltp_map = {}
        for opt in option_chain:
...
def _iron_fly_calc(option_chain, spot_price, config, lots):
    ivp = config.get("ivp", 50)
    avg_iv = config.get("avg_iv", 20)
    garch_vol = config.get("garch_vol", 20)
    regime_label = config.get("regime_label", ":smile: Neutral Volatility")
    factor = determine_hedge_width_factor(ivp, avg_iv, garch_vol, regime_label)
    wing = int(100 * factor)
...
def _iron_condor_calc(option_chain, spot_price, config, lots):
    ...
    atm = min(option_chain, key=lambda x: abs(x["strike_price"] - spot_price))
    short_wing_distance = int(100 * factor)
    long_wing_distance = int(200 * factor)
...
def _jade_lizard_calc(option_chain, spot_price, config, lots):
    ...
    atm = min(option_chain, key=lambda x: abs(x["strike_price"] - spot_price))
    call_strike = atm["strike_price"] + int(50 * factor)
    put_strike = atm["strike_price"]
    put_long_strike = atm["strike_price"] - int(100 * factor)
...
def _bull_put_spread_calc(option_chain, spot_price, config, lots):
    ...
    atm = min(option_chain, key=lambda x: abs(x["strike_price"] - spot_price))
    short_strike = atm["strike_price"] - int(50 * factor)
    long_strike = atm["strike_price"] - int(100 * factor)
...
def plot_allocation_pie(strategy_df, config):
    ...
    # Filter out rows where Capital Used <= 0
...
def plot_vega_distribution(full_chain_df):
    try:
        if full_chain_df.empty:
            st.info("No option chain data to plot Vega.")
            return
        
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(full_chain_df["Strike"], full_chain_df["Total Vega"], marker='o', color="#FFD700")
        ax.set_title("Vega Distribution by Strike", color="white")
        ax.set_xlabel("Strike", color="white")
        ax.set_ylabel("Total Vega", color="white")
        ax.tick_params(axis='x', colors='white', rotation=45)
        ax.tick_params(axis='y', colors='white')
        fig.patch.set_facecolor('#0E1117')
        ax.set_facecolor('#0E1117')
        ...
        st.warning(f":warning: Exception in Vega Distribution plot: {e}")

def plot_theta_decay(full_chain_df):
    try:
        if full_chain_df.empty:
            st.info("No option chain data to plot Theta.")
            return
        
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(full_chain_df["Strike"], full_chain_df["Total Theta"], marker='s', color="#00BFFF")
        ax.set_title("Theta Decay by Strike", color="white")
        ax.set_xlabel("Strike", color="white")
        ax.set_ylabel("Total Theta", color="white")
        ax.tick_params(axis='x', colors='white', rotation=45)
        ax.tick_params(axis='y', colors='white')
        fig.patch.set_facecolor('#0E1117')
        ax.set_facecolor('#0E1117')
        ...
        st.warning(f":warning: Exception in Theta Decay plot: {e}")
...
# --- Streamlit UI Setup ---
def render_metric_with_change(title, value, change, is_percent=True):
    change_str = f"{change:+.2f}%" if is_percent else f"₹{change:+.2f}"
    color = "#00FF7F" if change >= 0 else "#FF6347"
    return f"""
...
with st.sidebar:
    ...
    st.markdown('<div class="login-header">🔐 Volguard Login</div>', unsafe_allow_html=True)
    
    access_token = st.text_input("Enter Upstox Access Token", type="password", value=st.session_state.access_token, key="sidebar_token")
...
if st.session_state.logged_in and st.sidebar.button("Logout", use_container_width=True):
...
if st.session_state.logged_in and st.session_state.access_token:
    ...
    def load_all_data(config):
        ...
        config["ivp"] = ivp
        config["avg_iv"] = seller["avg_iv"]
        config["garch_vol"] = garch_7d
        config["regime_label"] = regime
        ...
        return (
            option_chain, spot_price, vix, nifty, seller, full_chain_df, market, ivp, hv_7, garch_7d, 
            xgb_vol, iv_rv_spread, iv_skew_slope, regime_score, regime, regime_note, regime_explanation, 
            event_df, strategies, strategy_rationale, event_warning, strategy_details, trades_df, 
            strategy_df, portfolio_summary, funds_data, sharpe_ratio
        )
    
    (option_chain, spot_price, vix, nifty, seller, full_chain_df, market, ivp, hv_7, garch_7d, 
     xgb_vol, iv_rv_spread, iv_skew_slope, regime_score, regime, regime_note, regime_explanation, 
     event_df, strategies, strategy_rationale, event_warning, strategy_details, trades_df, 
     strategy_df, portfolio_summary, funds_data, sharpe_ratio) = load_all_data(config)
    ...
    if 'prev_nifty' not in st.session_state:
        st.session_state.prev_nifty = nifty
    if 'prev_vix' not in st.session_state:
        st.session_state.prev_vix = vix
    if 'prev_atm_iv' not in st.session_state:
        st.session_state.prev_atm_iv = seller["avg_iv"]
    if 'prev_straddle_price' not in st.session_state:
        st.session_state.prev_straddle_price = seller["straddle_price"]
    
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.markdown(render_metric_with_change("Nifty 50 Spot", nifty, nifty - st.session_state.prev_nifty), unsafe_allow_html=True)
        st.session_state.prev_nifty = nifty
    
    with col2:
        st.markdown(render_metric_with_change("India VIX", vix, vix - st.session_state.prev_vix), unsafe_allow_html=True)
        st.session_state.prev_vix = vix
    ...
    with col4:
        st.markdown(render_metric_with_change("Straddle Price", seller['straddle_price'], seller['straddle_price'] - st.session_state.prev_straddle_price, False), unsafe_allow_html=True)
        st.session_state.prev_straddle_price = seller["straddle_price"]
    
    col5, col6, col7, col8 = st.columns(4)
    with col5:
        st.markdown(render_metric_with_change("ATM IV", seller['avg_iv'], seller['avg_iv'] - st.session_state.prev_atm_iv), unsafe_allow_html=True)
        st.session_state.prev_atm_iv = seller["avg_iv"]
    ...
    with tab2:
        ...
        plot_theta_decay(full_chain_df)
        
        st.subheader("Vega Distribution")
        plot_vega_distribution(full_chain_df)
        
        st.subheader("Option Chain Summary")
        styled_chain = full_chain_df.style.format({
            "Call IV": "{:.2f}", "Put IV": "{:.2f}", "IV Skew": "{:+.2f}",
            "Total Theta": "{:.2f}", "Total Vega": "{:.2f}", "Straddle Price": "{:.2f}", "Total OI": "{:.0f}"
        }).set_properties(**{"background-color": "#1A1C24", "color": "white"})
        st.dataframe(styled_chain, use_container_width=True)
        
        st.subheader("Theta/Vega Ratio Ranking")
        ...
        styled_eff = eff_df.style.format(precision=2).set_properties(**{"background-color": "#1A1C24", "color": "white"})
        st.dataframe(styled_eff, use_container_width=True)
